...
public class ImportConfig {
	...
	void importAccounts() throws IOException {
		if (accountService.getAllAccounts().iterator().hasNext()) {
		...
		if (Files.exists(data)) {
			try (final var lines = Files.lines(data)) {
				lines.map(it -> it.split(";"))
						...
						.forEach(it -> accountService.createAccount(it[0].trim(), it[1].trim(), it[2].trim(), "", 0, 0, true));
			}
		}
	}

	void importItems() throws IOException {
		if (itemService.getAllItems().iterator().hasNext()) {
		...
		if (Files.exists(data)) {
			try (final var lines = Files.lines(data)) {
				lines.map(it -> it.split(";"))
						...
						.forEach(it -> itemService.createItem(it[0].trim(), Integer.parseInt(it[1].trim()), it[2].trim(), it[3].trim(), Integer.parseInt(it[4].trim()), Boolean.parseBoolean(it[5].trim())));
			}
		} else {
